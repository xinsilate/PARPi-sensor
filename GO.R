...
setwd("/Users/sunx/Documents/ovca-immune/ICGC/goKEGG/")       #设置工作目录
rt=read.table("3583uniCoxResult.Sig169.limma.txt", header=T, sep="\t", check.names=F,row.names = 1)     #读取输入文件
...
#柱状图
pdf(file="barplot.go169.pdf", width=10, height=16)
...
#气泡图
pdf(file="bubble.go169.pdf", width=10, height=14)
...
pdf(file="barplot.color.go169.pdf", width=8, height=6.5)
...
#绘制圈图的主体部分
pdf("GO.circlize.169.pdf",width=10,height=10)
